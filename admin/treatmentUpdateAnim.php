    ...
    if(isset($_POST['nom']))
    {
        ...
        if($error==0)
        {
            if(empty($_FILES['fichier']['tmp_name']))
            {
                ...
                $update = $bdd->prepare("UPDATE animation SET nom=?, video=?, description=?, date=?WHERE id=?");
                $update->execute([$nom, $video,  $description, $date,$id]);
                $update->closeCursor();
                header("LOCATION:animation.php?update=".$id);
            }else{
                ...
                if($error == 0)
                {
                    ...
                    if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.$fichiercptl))
                    {
                        ...
                        $update = $bdd->prepare("UPDATE animation SET nom=?, video=?, description=?, date=?,fichier=? WHERE id=?");
                        $update->execute([$nom, $video, $description, $date,$fichiercptl,$id]);
                        $update->closeCursor();
                        header("LOCATION:animation.php?update=".$id);
                    }else{
                    ...
                    header("LOCATION:updateAnim.php?id=".$id."&errorimg=".$error);
                }     
            }
        }else{
            // redirection vers le formulaire en indiquant l'erreur
            header("LOCATION:updateAnim.php?id=".$id."&error=".$error);
        }
